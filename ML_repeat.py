...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 11 11:43:22 2023

@author: bt308570
"""

# take care of ZPVE comparison
# set up new MD if settings.txt is found
...
def find_topdir():
    ...
    while True:
        if not os.path.isfile(".anchor"):
            if "_ML" in os.path.basename(os.getcwd()):
                workdir = os.getcwd()
            os.chdir("..")
        else:
        ...
        if os.getcwd() == "/":
            ...
            sys.exit()
    os.chdir(origin)
    return topdir, workdir

# beginning of main program
if __name__ == '__main__':
    sys.stdout.flush()
    print("Running script to create ML-supported phonon calculations.")
    topdir, workdir = find_topdir()
    
    # origin = os.getcwd()
    while workdir != "Empty":
        if not "MD_" in os.path.basename(os.getcwd()):
            os.chdir("..")
        else:
            break
        if os.getcwd() == "/":
    ...
    if os.path.isfile("ML_settings.txt"):
        with open("ML_settings.txt",'r') as f:
            lines = f.readlines()
            for i in range(0,len(lines)):
                if "TEBEG" in lines[i]:
                    T = float(lines[i].split("=")[1])
                elif "EDIFF" in lines[i]:
                    EDIFF = lines[i].split("=")[1]
                elif "ACC" in lines[i]:
                    if "True" in lines[i]:
                        ACC = True
                    else:
                        ACC = False
                elif "ML_WTIFOR" in lines[i]:
                    ML_WTIFOR = float(lines[i].split("=")[1])
                elif "MAXMIX" in lines[i]:
                    MAXMIX = int(lines[i].split("=")[1])
                elif "LIMIT" in lines[i]:
                    max_steps = int(lines[i].split("=")[1])
                elif "NSW" in lines[i]:
                    ML_steps = int(lines[i].split("=")[1])
                elif "STEP" in lines[i]:
                    current_step = int(lines[i].split("=")[1])
                elif "ZPVE" in lines[i]:
                    ZPVE = []
                    n_values = int(lines[i+1])
                    for j in range(n_values):
                        ZPVE.append(float(lines[i+2+j]))
    else:
        ...
        for i in range(0,n):
           # reference structure
              if sys.argv[i] == "-t":
                  ...
                  ACC = False
              elif sys.argv[i] == "-a" or sys.argv[i] == "--accurate":
                  ACC = True
              ...
              elif sys.argv[i] == "-l" or sys.argv[i] == "--limit":
                  max_steps = int(sys.argv[i+1])
              elif sys.argv[i] == "-z" or sys.argv[i] == "--nsw":
                  ML_steps = int(sys.argv[i+1])
          
    
    if max_steps == 0:
        print("Please set the maximum amount of steps allowed for the fitting in total with -l or --limit.")
        sys.exit()
    if ML_steps == 0:
        print("Please set the amount of steps allowed for one MD run with -z or --nsw.")
        sys.exit()
...
# import the directory tools and settings
    ...
    if current_step < max_steps:
        ...
        tmp_dir = "MD_"+str(current_step)
        if not os.path.isdir("MD_"+str(current_step)):
            directory_tools.make_directory(tmp_dir)
            shutil.copyfile(workdir+"/POTCAR",tmp_dir+"/POTCAR")
            if os.path.isfile(workdir+"/ML_AB"):
                shutil.copyfile(workdir+"/ML_AB",tmp_dir+"/ML_AB")
            # MD settings
            os.chdir(workdir+"/"+tmp_dir)
            incar["NSW"] = ML_steps
            incar["ISYM"] = 0
            incar["ML_MODE"] = "train"
            ...
            kpoints.write_file("KPOINTS")
            poscar.write_file(name+"_MD.inp")
        else:
            os.chdir(workdir+"/"+tmp_dir)
        
        if os.path.isfile("VIB/thermal_properties.yaml") == False:
            with open("ML_settings.txt",'w') as f:
                f.write("TEBEG = "+str(T)+"\n")
                f.write("EDIFF = "+EDIFF+"\n")
                if ACC == True:
                    f.write("ACC = True"+"\n")
                f.write("ML_WTIFOR = "+str(ML_WTIFOR)+"\n")
                if MAXMIX > 0:
                    f.write("MAXMIX = "+str(MAXMIX)+"\n")
                f.write("LIMIT = "+str(max_steps)+"\n")
                f.write("NSW = "+str(ML_steps)+"\n")
                f.write("ZPVE\n")
                f.write(str(len(ZPVE))+"\n")
                for item in ZPVE:
                    f.write(str(item)+"\n")
                f.write("STEP = "+str(current_step)+"\n")

                    
        if os.path.isfile("run") or os.path.isfile("done"):
            print("MD running or done!")
        else:
            incar.write_file("run")
            # run VASP calculation
            print(subprocess.run(["/home/70/bt308570/bin/vsub_ML "+name+"_MD.inp"], shell=True, stdout=subprocess.PIPE))
        if os.path.isfile("done") and not os.path.isfile("REFIT/done") and not os.path.isfile("REFIT/run") :
            ...
            kpoints.write_file("KPOINTS")
            poscar.write_file(name+"_REFIT.inp")
            os.chdir(workdir+"/"+tmp_dir)
        if os.path.isfile("REFIT/done") and not os.path.isfile("VIB/done") and not os.path.isfile("VIB/run") : 
            ...
            kpoints.write_file("KPOINTS")
            shutil.copyfile(workdir+"/OPOSCAR","POSCAR")
            os.chdir(workdir+"/"+tmp_dir)
        ...
        if os.path.isfile("done") and os.path.isfile("OUTCAR"):
            with open("OUTCAR", "r") as file:
                for line in file:
                    if "Total C" in line:
                        time_ml[0] = time_ml[0] + float(line.split(":")[1])
            ...
            if os.path.isfile(os.getcwd()+"/REFIT/run") or os.path.isfile(os.getcwd()+"/REFIT/done"):
                ...
                if os.path.isfile(os.getcwd()+"/REFIT/done") and os.path.isfile(os.getcwd()+"/REFIT/OUTCAR"):
                    with open(os.getcwd()+"/REFIT/OUTCAR", "r") as file:
                        for line in file:
                            if "Total C" in line:
                                time_ml[1] = time_ml[1] + float(line.split(":")[1])
            elif os.path.isfile("done"):
                if os.path.isfile("ML_ABN"):
                    shutil.copyfile("ML_ABN","REFIT/ML_AB")
                    shutil.copyfile("ML_ABN",workdir+"/ML_AB")
                    shutil.copyfile("CONTCAR","REFIT/POSCAR")
                    os.chdir("REFIT")
                    incar.write_file("run")
                    # run VASP calculation
                    print(subprocess.run(["/home/70/bt308570/bin/vsub_ML "+name+"_REFIT.inp"], shell=True, stdout=subprocess.PIPE))
                    os.chdir("..")
            if os.path.isfile("REFIT/done") and os.path.isfile("REFIT/ML_FFN"):
                shutil.copyfile("REFIT/ML_FFN","VIB/ML_FF")
                os.chdir("VIB")
                structure = Structure.from_file(workdir+"/POSCAR")
                ...
                for j in range(1,max(numbers)+1):
                    ...
                    if os.path.isfile(disp_dir+"/run"):
                        ...
                        with open(disp_dir+"/OUTCAR", "r") as file:
                            for line in file:
                                if "Total C" in line:
                                    time_ml[2] = time_ml[2] + float(line.split(":")[1])
                ...
                if os.path.isfile("thermal_properties.yaml") == True:
                    ...
                    with open('thermal_properties.yaml','r') as file:
                        for line in file:
                            ...
                            if 'zero_point' in line:
                                # transform kJ/mol per unit cell into meV per atom
                                tmp_ZPVE = float(line.split()[1])/96.49*1000
                    ...
                    ZPVE.append(tmp_ZPVE/num_atoms)
                    
                    res_dir = (topdir+"/RESULTS/"+name+"_ML_"+str(current_step+ML_steps))
                    ...
                    if append == True:
                        with open("ML_settings.txt",'w') as f:
                            ...
                            f.write("LIMIT = "+str(max_steps)+"\n")
                            f.write("NSW = "+str(ML_steps)+"\n")
                            f.write("STEP = "+str(current_step+ML_steps)+"\n")
                            f.write("ZPVE\n")
                            f.write(str(len(ZPVE))+"\n")
                            for item in ZPVE:
                                f.write("{:9.3f}".format(item)+"\n")
                            f.write("VIBDONE\n")
                        print(subprocess.run(["/home/70/bt308570/pymatgen_scripts/ML_repeat.py"], shell=True, stdout=subprocess.PIPE))
                os.chdir(workdir+"/"+tmp_dir)
